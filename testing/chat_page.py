...
class ChatPage(GridLayout):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        # dividing layout into one column then into two rows.
        self.cols = 1
        self.rows = 2

        # using Window to give element height of 90% the current window size
        self.history = Label(height=Window.size[1] * 0.9, size_hint_y=None)
        self.add_widget(self.history)

        # diving the row(acc to order row will come now) into 80% - 20%
        self.new_message = TextInput(width=Window.size[0] * 0.8,
            size_hint_x=None, multiline=False)
        self.send = Button(text="Send")
        self.send.bind(on_press=self.send_message)

        # creating a layout inside a layout and adding two more columns
